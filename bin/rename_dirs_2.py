...
"""
 function to rename the tree dirs for CAZy families with subgroups
"""
import os

def rename_dirs(model, directory, fams):
    """
    rename_dirs function renames teh treedirs so
    ...
    for families in os.listdir(directory):
        if families in fams:
            for groups in os.listdir(os.path.join(directory, families)):
                if groups.startswith("g"):
                    data_dir = os.path.join(directory, families, groups, "positive_selection", model)
                    for treedirs in os.listdir(data_dir):
                        trees = os.path.join(data_dir, treedirs)
                        os.rename(trees, trees +"_mlc")

# list of cazy families we dont want to apply teh function on
group_fams = ["CBM48", "CBM50", "CE8", "GH1", "GH3", "GH13",
              "GH19", "GH23", "GH28", "GH33", "GH73", "GH103",
              "GH104", "GH105", "GT1", "GT2", "GT4", "GT9", "GT35",
              "GT51", "PL1", "PL3", "PL4", "PL9"]
